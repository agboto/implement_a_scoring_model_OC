...
def select_col_by_type(df, col_type):
    return list(df.select_dtypes(col_type))
...
def imputer_nan(df, strategy, var_type):
    ...
    pd.DataFrame(array_type, columns=var_type)
    return pd.DataFrame(array_type, columns=var_type)
...
def trans_func(df):
    ...
    pd.concat([df_num, dummies], axis=1)
    #concatenate_df.sample(n=500, random_state=1)
    #print('Bonjour 2')
...
def main():
    ...
    html_temp = """
       <div style="background-color:blue;padding:12px; border-radius:15px">
       <h2 style="color:white;text-align:center;"> OUTIL D'AIDE A LA DECISION D'OCTROI DE CREDIT </h2>
       </div>
       """
    st.set_page_config(
    ...
    ML_URI = "https://credit-scoring-fastapi.herokuapp.com/predict"
  

    
    "\n"
    st.sidebar.image("image_oc.png")
    ...
    if id_client:
        ...
        if st.checkbox('La décision finale'):
            if solva > 0.5:
                st.success("CLIENT ELIGIBLE AU PRET")
            else:
                st.error("CE CLIENT EST A RISQUE")
